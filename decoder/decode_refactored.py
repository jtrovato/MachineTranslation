...
for french_sentence in french:
  ...
  for french_sentence_p in reorder(french_sentence, 0):
    ...
    stacks = [{} for _ in french_sentence_p] + [{}]
        ...
        for j in xrange(i+1,len(french_sentence_p)+1):
          ...
          if french_sentence_p[i:j] in tm:
            ...
            for phrase in tm[french_sentence_p[i:j]]:
              ...
              logprob += lm.end(current_lm_state) if j == len(french_sentence_p) else 0.0
    ...
    p_max = max(stacks[-1].itervalues(), key=lambda h: h.logprob)
    if p_max > winner:
      winner = p_max
      sys.stderr.write(str(winner.logprob))
